...
namespace DAQLKS
{
    public partial class QuanLyNhanVien : Form
    {
        ...
        {
            int i = dataGirdViewDSNhanVien.CurrentRow.Index;
            String maDelete = dataGirdViewDSNhanVien.Rows[i].Cells[0].Value.ToString();

            DialogResult dialogResult = MessageBox.Show("Bạn có chắc chắn muốn xóa nhân viên này không?", "Xác nhận xóa", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        private void dataGirdViewDSNhanVien_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                int i;
